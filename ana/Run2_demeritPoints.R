...
getR2ParticipantPts <- function(group, id, session) {
...
getR2GroupPts <- function(group, session){
  ...
  for(pp in pp_group){
    ppdat <- getR2ParticipantPts(group = group, id = pp, session = session)
    trial <- ppdat$trialno
    dempts <- ppdat$dempts
...
getR2S2GroupPts <- function(group, session = 2){
...
getR2AllTrackGroupPts<- function(groups = c('T-RACING_0', 'T-RACING_180', 'T-RACING_90', 'T-RACING_270'), session = 1, target = 0){
  
  alldata <- data.frame()
  
  for(group in groups){
    data <- getR2GroupPts(group = group, session = session)
    trial <- data$trial
  ...
  if(target == 1){
    write.csv(ndat, file=sprintf('data/Run2_DemeritPoints_AllTrack_S%03d.csv', session), row.names = F) 
  } else {
    return(ndat)
  }
}

getR2AllTrackGroupPtsCI <- function(session = 1, type = 'b'){
  
  data <- getR2AllTrackGroupPts(session = session)
  trialno <- data$trial
  data1 <- as.matrix(data[,2:(dim(data)[2])])
  ...
  write.csv(confidence, file=sprintf('data/Run2_DemeritPointsCI_AllTrack_S%03d.csv', session), row.names = F) 
  
}

plotR2AllTrackPts <- function(session = 1, target='inline') {
  
  
  #but we can save plot as svg file
  if (target=='svg') {
    svglite(file='doc/fig_run2/Fig17_DemeritPoints_AllTrack.svg', width=12, height=7, pointsize=14, system_fonts=list(sans="Arial"))
  }
  
  # create plot
  #NA to create empty plot
  plot(NA, NA, xlim = c(0,301), ylim = c(0, 8), 
  ...
  axis(2, at = c(0, 1, 2, 3, 4, 5, 6, 7, 8), las=2) #tick marks for y axis
  
  
  #read in files created by CI function
  groupconfidence <- read.csv(file=sprintf('data/Run2_DemeritPointsCI_AllTrack_S%03d.csv', session))
...
getR2S2AllTrackGroupPts<- function(groups = c('T-RACING_0', 'T-RACING_180', 'T-RACING_90', 'T-RACING_270'), session = 2, target = 0){
  
  alldata <- data.frame()
  
  for(group in groups){
    data <- getR2S2GroupPts(group = group, session = session)
    trial <- data$trial
    data1 <- as.matrix(data[,2:(dim(data)[2])])
...
getR2S2AllTrackGroupPtsCI <- function(session = 2, type = 'b'){
  
  data <- getR2S2AllTrackGroupPts(session = session)
  trialno <- data$trial
  data1 <- as.matrix(data[,2:(dim(data)[2])])
...
plotR2S2AllTrackPts<- function(session = 2, blocks = c(1,2,3,4), target='inline') {
  
  
  #but we can save plot as svg file
  if (target=='svg') {
    svglite(file='doc/fig_run2/Fig17A_DemeritPoints_AllTrack_Session2.svg', width=12, height=7, pointsize=14, system_fonts=list(sans="Arial"))
  }
  
  # create plot
  #NA to create empty plot
  plot(NA, NA, xlim = c(0,121), ylim = c(0, 8), 
       xlab = "Trial", ylab = "Demerit points", frame.plot = FALSE, #frame.plot takes away borders
       main = sprintf("Mean demerit points across trials: Session %s", session), xaxt = 'n', yaxt = 'n') #xaxt and yaxt to allow to specify tick marks
...
getR2BlockedDemPts <- function(session, blockdefs) {
  
  dat <- read.csv(file=sprintf('data/Run2_DemeritPoints_AllTrack_S%03d.csv', session))
...
getR2DemPtsSession1 <- function(){
  blockdefs <- list('S1_first'=c(2,5), 'S1_last'=c(295,6))
  LC_part1 <- getR2BlockedDemPts(session = 1, blockdefs=blockdefs) 
